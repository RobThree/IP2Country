    ...
    public class CachingWebClient : WebClient
    {
        ...
        public async Task<string> DownloadAsync(Uri url, string path, TimeSpan? ttl = null)
        {
            ...
            if (IsFileExpired(path, ttl ?? DefaultTTL))
            {
                ...
                await (await contentstream.ConfigureAwait(false)).CopyToAsync(filestream).ConfigureAwait(false);
            }
            return path;
        }

        private static bool IsFileExpired(string path, TimeSpan ttl)
            => (!File.Exists(path) || (DateTime.UtcNow - new FileInfo(path).LastWriteTimeUtc) > ttl);
    }
}
